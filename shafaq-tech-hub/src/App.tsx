  ...
  return (
    <div className="app">
      ...
      <main className="wrap">
        ...
        {/* 🌟 3 HIGH-CONVERTING VALUE PROPOSITION CARDS (سودىڭىزنى زامانىۋى تېخنىكا بىلەن...) */}
        {page === "home" && (
          <section className="section cta-banner-sec">
            <div className="grid g3 cta-banner-grid">
              {t.ctaBanner.cards.map((card, ci) => {
                const cardGlow =
                  ci === 0
                    ...
                    ? "rgba(217, 119, 6, 0.35)"
                    : "rgba(22, 163, 74, 0.35)";
                const cardBorder = ci === 0 ? "#8b5cf6" : ci === 1 ? "#d97706" : "#16a34a";
                const cardBgGrad =
                  ci === 0
                    ? "linear-gradient(145deg, rgba(124, 58, 237, 0.22), rgba(18, 20, 30, 0.88))"
                    : ci === 1
                    ? "linear-gradient(145deg, rgba(217, 119, 6, 0.22), rgba(18, 20, 30, 0.88))"
                    : "linear-gradient(145deg, rgba(22, 163, 74, 0.25), rgba(18, 20, 30, 0.88))";
                const cardBgLight =
                  ci === 0
                ...
                return (
                  <div
                    className={`card cta-prop-card cta-prop-${ci}`}
                    key={ci}
                    style={{
                      background: isDark ? cardBgGrad : cardBgLight,
                      border: `2px solid ${cardBorder}`,
                      boxShadow: `0 14px 34px ${isDark ? cardGlow : "rgba(0,0,0,0.06)"}`,
                    }}
                  >
                    <div className="cta-prop-top">
                      <span
                        className="cta-prop-badge"
                        style={{
                      ...
                      >
                        <span style={{ fontSize: 16 }}>{card.icon}</span> {card.badge}
                      </span>
                    </div>

                    <p
                      className="cta-prop-text"
                      style={{
                        color: isDark
                          ? ci === 0
                            ? "#f3e8ff"
                  ...
                  {"btnText" in card && (card as any).btnText && (
                    <button
                      className="btn cta-prop-action-btn"
                      style={{
        ...
        <section className="section direct-contact-sec">
          <div className="card direct-card">
            ...
            <div className="direct-buttons-row">
              ...
              <button
                type="button"
                className="direct-btn wc-btn"
                onClick={() => setWechatModal(true)}
              >
                <span className="btn-icon">🟢</span>
                <span>{t.wechatBtn}</span>
              </button>
              <button
                type="button"
